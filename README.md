...
- [url_shortener](#urlshortener)
  ...
  - [Stress test results](#stress-test-results)
    - [V1. Use auto-increment key](#v1-use-auto-increment-key)
      - [POST](#post)
      - [GET](#get)
    - [V2 Use snowflake UUID](#v2-use-snowflake-uuid)
    - [V3 Use redis as message queue](#v3-use-redis-as-message-queue)
<!--toc:end-->
...
The service is [online](https://short.vinchent.xyz).

It takes a link and returns a link. I don't know if it is really shorter though.

The backend handles a `POST` request to convert a URL to a random string
(using `snowflake` algorithm), and then handles a `GET` request to redirect
to the original URL.

I also added a `/count` endpoint to see the click count for a given short URL.

Plus, there is another cron service that removes outdated URLs. I configured
to 1 month, so the links below might not be accessible when you see this page.

Examples: 

```bash
<@url_shortener>-<⎇ main>-<±>-> http -v POST https://short.vinchent.xyz/api url=https://pkg.go.dev
POST /api HTTP/1.1
...
Access-Control-Allow-Origin: http://short.vinchent.xyz
Connection: keep-alive
Content-Length: 52
Content-Type: application/json
Date: Sun, 19 Jan 2025 21:28:09 GMT
Server: nginx/1.18.0
Vary: Origin
X-Ratelimit-Limit: 5
...
{
    "url": "https://short.vinchent.xyz/api/2ExPvBDgg1w"
}
<@url_shortener>-<⎇ main>-<±>-> http -v GET "https://short.vinchent.xyz/api/2ExPvBDgg1w"
GET /api/2ExPvBDgg1w HTTP/1.1
Accept: */*
Accept-Encoding: gzip, deflate, br
Connection: keep-alive
Host: short.vinchent.xyz
...
Connection: keep-alive
Content-Length: 0
Date: Sun, 19 Jan 2025 21:28:48 GMT
...
<@url_shortener>-<⎇ main>-<±>-> http -v GET "https://short.vinchent.xyz/api/2ExPvBDgg1w/count"
GET /api/2ExPvBDgg1w/count HTTP/1.1
Accept: */*
Accept-Encoding: gzip, deflate, br
Connection: keep-alive
Host: short.vinchent.xyz
...
Content-Length: 106
Content-Type: application/json
Date: Sun, 19 Jan 2025 21:29:33 GMT
Server: nginx/1.18.0
Vary: Origin
X-Ratelimit-Limit: 5
...
{
    "count": 5,
    "created_at": "2025-01-19T21:28:09.056042Z",
    "id": 1881091297872187392,
    "url": "https://pkg.go.dev"
}
```
...
- Stress test: `wrk`
- Profiling: `pprof`
- Simple Message Queue: `Redis List`
- Frontend: HTTP+Vanilla javascript (generated by [claude](claude.ai))
